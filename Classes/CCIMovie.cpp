...
NS_CC_EXT_BEGIN
static __Dictionary * movies = NULL;
CCIMovie * CCIMovie::decodeFromFile(std::string filename){
    
    if (!movies) {
        movies = new __Dictionary();
    }
    ...
    auto fullPath = FileUtils::getInstance()->fullPathForFilename(filename.c_str());
    int fileSize = 0;
    
    unsigned char * swfdata = FileUtils::getInstance()->getFileData(fullPath, "rb", (ssize_t *)(&fileSize));
    ...
    movie = new CCIMovie();
    movie->frameLabels = __Dictionary::create();
...
__Dictionary * CCIMovie::getFrameLabels(){
