...
bool CCINode::initWithFile(std::string cci_filename){
    ...
    __Dictionary * nodes = __Dictionary::create();
    this->initWithTags(swfMovie->getTags(), swfMovie,nodes);
    this->setAnchorPoint(Point(0.5, 0.5));
    return true;
}
bool CCINode::initWithTags(std::vector<CCIMovieTag *> tags,CCIMovie * swfMovie,__Dictionary * nodes){
    
    this->textures = __Dictionary::create();
    ...
    this->setAnchorPoint(Point(0, 1));//ajust position for moveclips
    this->loadImages();
...
CCINode * CCINode::createWithTags(std::vector<CCIMovieTag *> tags,CCIMovie * swfMovie,__Dictionary * nodes){
    CCINode * ret = new CCINode();
...
void CCINode::showFrame(float dt){
...
void CCINode::applyColorTransform(cocos2d::CCNode *node, Color4B colorTransform){
    auto children =  node->getChildren();
    for (int i=0; i<node->getChildrenCount(); i++) {
        CCNode * childNode = (CCNode *)children.at(i);
        ...
        for (int i=0; i<fillStyles->styleCount; i++) {
            ...
            if (fillStyle&&fillStyle->BitmapId) {
                ...
                if (texture2d) {
                    ...
                    //sprite->setAnchorPoint(Point(-0.5, 1.5));
